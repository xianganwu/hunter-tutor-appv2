...
- **Safety nets must distinguish data provenance.** A "reject what you can't verify" safety net must know whether the math was computed by the AI (needs verification) or by code (correct by construction). Seeded questions should verify against the seed, not against regex parsing of English. The safety net at `validate-question.ts:1030-1049` rejects valid seeded place-value questions at tier 5 because the AI uses creative word-problem formats that the regex detectors can't parse — even though the answer was pre-computed correctly.
- **Validation strictness must scale with data uncertainty.** Code-computed seeds need format checks only. AI-computed answers need mathematical verification. Unrecognized formats from AI need rejection. Treating all sources identically either over-rejects (blocking valid seeded questions) or under-rejects (passing bad AI-computed answers).
- **Retry mechanisms must vary the approach, not just the constraints.** Retrying with relaxed dedup pressure doesn't help when the failure is in validation, not generation. Effective retries should also try: lower difficulty tiers, constrained prompt formats, or relaxed validation for seeded data.
